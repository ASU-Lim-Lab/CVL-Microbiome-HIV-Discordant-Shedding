...
##Mann-Whitney tests of pathway Bray-Curtis dissimilarity within and between groups. Input: table with Bray-Curtis dissimilarity and metadata for sample pairs.##
...
##Mann-Whitney tests of gene family Bray-Curtis dissimilarity within and between groups. Input: table with Bray-Curtis dissimilarity and metadata for sample pairs.##
stats.data.gene <- read.delim("gene_relab_BC_long_format_metadata.txt", header = TRUE, sep = "\t")
w_b.ctrls.gene<-subset(stats.data.gene,comparison_all =='Control.within'|comparison_all =='Control.between')
w_b.shed.gene<-subset(stats.data.gene,comparison_all =='Shedder.within'|comparison_all =='Shedder.between')
w.ctrl_shed.gene<-subset(stats.data.gene,comparison_all =='Control.within'|comparison_all =='Shedder.within')
b.ctrl_shed.gene<-subset(stats.data.gene,comparison_all =='Control.between'|comparison_all =='Shedder.between')
w.shed.DSC_nonDSC.gene<-subset(stats.data.gene,comparison_DSC_vs_notDSC =='Shedder.nonDSC.nonDSC'|comparison_DSC_vs_notDSC =='Shedder.DSC.nonDSC')
w.ctrl.DSC_nonDSC.gene<-subset(stats.data.gene,comparison_DSC_vs_notDSC =='Control.nonDSC.nonDSC'|comparison_DSC_vs_notDSC =='Control.DSC.nonDSC')
mw.w_b.ctrls.gene<-wilcox.test(bray_curtis~comparison_all, data = w_b.ctrls.gene, alternative = c("two.sided"))
mw.w_b.shed.gene<-wilcox.test(bray_curtis~comparison_all, data = w_b.shed.gene, alternative = c("two.sided"))
mw.w.ctrl_shed.gene<-wilcox.test(bray_curtis~comparison_all, data = w.ctrl_shed.gene, alternative = c("two.sided"))
mw.b.ctrl_shed.gene<-wilcox.test(bray_curtis~comparison_all, data = b.ctrl_shed.gene, alternative = c("two.sided"))
mw.w.shed.DSC_nonDSC.gene<-wilcox.test(bray_curtis~comparison_DSC_vs_notDSC, data = w.shed.DSC_nonDSC.gene, alternative = c("two.sided"))
mw.w.ctrl.DSC_nonDSC.gene<-wilcox.test(bray_curtis~comparison_DSC_vs_notDSC, data = w.ctrl.DSC_nonDSC.gene, alternative = c("two.sided"))
bc.test.gene<-c("mw.w_b.ctrls.gene","mw.w_b.shed.gene","mw.w.ctrl_shed.gene","mw.b.ctrl_shed.gene","mw.w.shed.DSC_nonDSC.gene","mw.w.ctrl.DSC_nonDSC.gene")
p.val.gene<-c(mw.w_b.ctrls.gene$p.value,mw.w_b.shed.gene$p.value,mw.w.ctrl_shed.gene$p.value,mw.b.ctrl_shed.gene$p.value,mw.w.shed.DSC_nonDSC.gene$p.value,mw.w.ctrl.DSC_nonDSC.gene$p.value)
bc.pvals.gene<-data.frame(bc.test.gene,p.val.gene)
pval.adjust.gene <- p.adjust(bc.pvals.gene$p.val.gene, method = "BH")
